...
Deckbox.ui.Tooltip.prototype = {
    _padContent: function(content) {
        return "<table style=\"max-width:50px;\"><tr><td>" + content + "</td><th style='background-position: right top;'></th></tr><tr>" +
...
Deckbox._ = {
    ...
    scrollOffsets: function() {
		...
		// HIER OFFSET EINBAUEN
	    window.pageYOffset+1 || document.documentElement.scrollTop  || document.body.scrollTop
        ];
    },
...
/*        if (el.className.match('no_tooltip')) return false;
        return true;//href.match(/^https?:\/\/[^\/]*\/(mtg|wow|whi)\/.+/);*/
	if (el.className.match('simple')) return true;
return false;
    },
...
(function() {
	...
	var XXX = event.target.getAttribute('data-tooltip');
        if (Deckbox._.needsTooltip(el)) {
            ...
            if (!no) {
                ...
                if (url = el.getAttribute('data-tt')) {
                    ...
                    showImage(el, XXX + '/tooltip', posX, posY);
                } 
            }
        }
    }
